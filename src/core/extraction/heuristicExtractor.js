 ...
 */
// Une ligne n'est traitee comme une LISTE de mots-cles que si elle
// ressemble vraiment a une liste (delimiteurs presents, ou courte) — sinon
// c'est une phrase de prose (ex. section "Competences" suivie d'un
// paragraphe libre sans nouvel en-tete reconnu) qu'il ne faut PAS decouper
// mot par mot, au risque de produire des centaines de "mots-cles" bruit.
const LIST_LINE_MAX_LENGTH = 120;
const MAX_SKILL_FACTS = 40; // plafond ATS : au-dela, la liste n'aide plus le matching, elle le noie.

function looksLikeList(line) {
  if (line.length > LIST_LINE_MAX_LENGTH) return false;
...
export function extractFacts(doc) {
  ...
  for (const line of sections.skills || []) {
    if (skillCount >= MAX_SKILL_FACTS) break;
    if (!looksLikeList(line)) continue; // paragraphe de prose, pas une liste : on l'ignore plutot que de le decouper mot par mot
