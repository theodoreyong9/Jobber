...
export const Domain = Object.freeze({ JOB: 'job', DATING: 'dating', MISSION: 'mission', DRIVE: 'drive' });
...
export function createCandidateBroadcast({ domain = Domain.JOB, senderId, displayName, searchKeywords, skills, cities, countries, yearsOfExperience, yearsOfExperienceEstimated, age, link, lat, lng, cvFileName }) {
  return {
...
export function createChatRequest({ domain = Domain.JOB, toPeerId, toInstanceId, roomTitle, fromName, fromId }) {
  return { ...base(MessageType.CHAT_REQUEST), domain, toPeerId, toInstanceId: toInstanceId || null, roomTitle: roomTitle || null, fromName: fromName || null, fromId: fromId || null };
}

export function createMeetingProposal({ domain = Domain.JOB, toPeerId, toInstanceId, roomTitle, note, fromName, fromId }) {
  return { ...base(MessageType.MEETING_PROPOSAL), domain, toPeerId, toInstanceId: toInstanceId || null, roomTitle: roomTitle || null, note: String(note || '').slice(0, 500), fromName: fromName || null, fromId: fromId || null };
}
