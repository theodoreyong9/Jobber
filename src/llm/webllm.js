...
import { buildSemanticAnalysisPrompt, buildDisambiguationPrompt, buildKeywordBoostPrompt } from './prompts.js';
...
/**
 * "Boost" optionnel côté candidat (§ demande : IA côté candidat, avant
 * l'envoi — pas côté annonceur). Suggère des mots-clés additionnels à
 * partir du texte intégral du CV, en plus de ceux déjà trouvés par le CPU.
 * Échoue proprement (renvoie `ok: false`) sur tout problème — JSON
 * invalide, timeout, sortie vide — sans jamais lever d'exception : le
 * candidat doit toujours pouvoir passer en direct avec ses seuls mots-clés
 * CPU si le boost échoue.
 * @param {WebLLMHandle} handle
 * @param {{ cvText: string, currentKeywords: string[] }} params
 * @returns {Promise<{ ok: true, keywords: string[] } | { ok: false }>}
 */
export async function runKeywordBoost(handle, params) {
  try {
    const messages = buildKeywordBoostPrompt(params);
    const raw = await handle.chat(messages);
    const parsed = parseJsonLoose(raw);
    if (!Array.isArray(parsed.keywords)) return { ok: false };
    const keywords = parsed.keywords
      .filter((k) => typeof k === 'string' && k.trim().length > 0 && k.length <= 60)
      .slice(0, 10);
