...
import { tokenize, matchTokens, explain, extractEarliestYear, MATCHING_ENGINE_VERSION, MAX_KEYWORDS } from '../js/matching.js';
...
test('tokenize caps to MAX_KEYWORDS even on a very long document — the real fix for "273 keywords from one CV"', () => {
  const words = Array.from({ length: 200 }, (_, i) => `uniqueword${i}`).join(' ');
  const t = tokenize(words);
  assert.equal(t.length, MAX_KEYWORDS);
});

test('tokenize keeps the most frequent terms when capping, not an arbitrary subset', () => {
  const filler = Array.from({ length: 100 }, (_, i) => `filler${i}`).join(' ');
  const text = `important important important important important ${filler}`;
  const t = tokenize(text);
  assert.ok(t.includes('important'));
  assert.equal(t[0], 'important'); // most frequent term ranks first regardless of the cap
});
