...
## Modes, un seul affiché à la fois

Des paires, groupées par le même fonctionnement interne (chercheur = dépose
un document et diffuse ; offreur = publie une ou plusieurs salles, et
décide qui contacter) :

| Paire | Chercheur | Offreur (décide qui contacter) |
|---|---|---|
| Emploi | 🧑‍💼 **Candidat** (CV) | 🏢 **Employeur** (poste) |
| Mission | 📣 **Annonceur** (propal + texte + prix) | 🤝 **Client** (besoin) |
| Trajet | 🚗 **Conducteur** (trajet, document optionnel) | 🧍 **Passager** (décide) |

Et un mode symétrique (tout le monde diffuse à la fois ce qu'il est et ce
qu'il recherche ; **les deux côtés doivent accepter** pour discuter, pas de
rôle chercheur/offreur) : 💞 **Rencontre** — profil + âge (affiché, hors
score, jamais utilisé pour comparer).

Chaque recherche/salle/profil peut, en option, **partager une position
précise** (opt-in explicite, jamais automatique) — c'est ce qui alimente le
super-mode Near ci-dessous.

## Deux super-modes transversaux

- **📍 Near** : agrège tous les résultats déjà découverts par les modes
  offreurs/symétriques actifs (salles employeur/client/passager, profils
  rencontre) et ne garde que ceux à l'intérieur d'un rayon choisi via une
  jauge. Filtre uniquement les résultats dont l'**autre côté a lui-même
  choisi** de partager sa position — pas de géocodage de ville, pas
  d'annuaire tiers, juste une distance entre deux coordonnées explicites
  (formule de haversine, `src/core/geo.js`). Se localiser soi-même est
  entièrement optionnel et réversible (bouton "Arrêter le partage").
- **🤖 Agent (prototype)** : parcourt ce qui est déjà actif dans les autres
  modes (conversations non lues, salles avec des profils correspondants,
  salles vides à élargir) et propose des actions. **À base de règles
  simples, explicitement pas une IA** — une vraie base pour un futur agent
  plus élaboré, pas encore ça.
