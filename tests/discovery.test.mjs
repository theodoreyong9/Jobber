...
test('RoomRanker score une diffusion candidat pertinente et la classe (compte brut de mots-cles)', () => {
  ...
  ranker.ingestBroadcast('cand1', { peerId: 'cand1', searchKeyword: 'python', displayName: 'Jean', skills: ['python'], cvFileName: 'cv.docx', timestamp: 1 });
...
test('candidateBroadcastToComparable ne fabrique jamais de texte integral et porte ville/anciennete', () => {
  const comparable = candidateBroadcastToComparable({ peerId: 'p1', skills: ['sql'], city: 'Lyon', yearsOfExperience: 4 });
  assert.equal(comparable.id, 'p1');
  assert.equal(comparable.city, 'lyon');
